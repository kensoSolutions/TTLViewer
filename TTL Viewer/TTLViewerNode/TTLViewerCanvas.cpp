...
void TTLViewerCanvas::openSpikePlot(bool select)
{
	ttlViewer->showingSpikesPlots(select);
}
...
void TTLViewer::showingSpikesPlots(bool select)
{
	if (select){
		for (int i = spikePlots.size() - 1; i >= 0; i--)
		{
			if (spikePlots[i]->isSelected){
				//spikePlotsSaved.add(getSpikePlot(i));
			}
			else
			{
				spikePlots[i]->setVisible(false);
			}
		}
		resized();
	}
    ...
    if (spikePlots.size() > 0)
    {
        ...
        for (int i = 0; i < spikePlots.size(); i++)
        {
            ...
            if (spikePlots[i]->nChannels == 1)
            {
				...
				height = jmax(((float)h *  spikePlots[i]->aspectRatio) / (float) spikePlots.size(), ((float)h *  spikePlots[i]->aspectRatio)/32.0f);
            ...
            spikePlots[i]->setBounds(width*column, row*(int)height, width, (int)height);
...
void SpikePlot::paint(Graphics& g)
{
   ...
   // g.drawRect(0,0,getWidth(),getHeight());
	...
	String subname = name.substring(name.length() - 3, name.length());
	if (subname[0] == ' ') subname = subname.substring(subname.length() - 2, subname.length());

	g.drawText(subname, 10, 2, 800, 20, Justification::left, false);

}

void SpikePlot::mouseDoubleClick(const MouseEvent& event)
{
	//Cambios para seleccionar y mostrar en la pantalla un solo plot
	if (event.mods.isLeftButtonDown()){
		if (!isSelected){
			select();
			canvas->openSpikePlot(true);
			x = getX();
			y = getY();
			w = getWidth();
			h = getHeight();
...
void SpikePlot::resized()
{
    ...
    for (int i = 0; i < nWaveAx; i++)
    {
        wAxes[i]->setBounds(40 + (i % nWaveCols) * axesWidth/nWaveCols, 0 + (i/nWaveCols) * axesHeight, axesWidth/nWaveCols, axesHeight);
    ...
    for (int i = 0; i < nProjAx; i++)
        pAxes[i]->setBounds(40 + (1+i%nProjCols) * axesWidth, 0 + (i/nProjCols) * axesHeight, axesWidth, axesHeight);
...
void WaveAxes::mouseMove(const MouseEvent& event)
{
    ...
    {
        thresholdColour = Colours::lightyellow;
