...
TTLViewerNode::TTLViewerNode()
    : GenericProcessor  ("TTL Viewer")
    , displayBufferSize (5)
...
AudioProcessorEditor* TTLViewerNode::createEditor()
{
    editor = new TTLViewerEditor (this);
    return editor;
}


void TTLViewerNode::updateSettings()
{
    //std::cout << "Setting num inputs on TTLViewerNode to " << getNumInputs() << std::endl;
...
bool TTLViewerNode::enable()
{
    std::cout << "TTLViewerNode::enable()" << std::endl;
    TTLViewerEditor* editor = (TTLViewerEditor*) getEditor();
...
bool TTLViewerNode::disable()
{
    std::cout << "TTLViewerNode disabled!" << std::endl;
...
int TTLViewerNode::getNumberOfChannelsForElectrode (int i) const
{
...
String TTLViewerNode::getNameForElectrode (int i) const
{
    if (i > -1 && i < electrodes.size())
    {
...
void TTLViewerNode::addSpikePlotForElectrode (SpikePlot* sp, int i)
{
...
void TTLViewerNode::removeSpikePlots()
{
...
int TTLViewerNode::getNumElectrodes() const
{
    return electrodes.size();
}


void TTLViewerNode::startRecording()
{
    setParameter (1, 0.0f); // need to use the 'setParameter' method to interact with 'process'
}


void TTLViewerNode::stopRecording()
{
    setParameter (0, 0.0f); // need to use the 'setParameter' method to interact with 'process'
}


void TTLViewerNode::setParameter (int param, float val)
{
    //std::cout<<"TTLViewerNode got Param:"<< param<< " with value:"<<val<<std::endl;
...
void TTLViewerNode::process (AudioSampleBuffer& buffer)
{
...
/*
void TTLViewerNode::handleEvent(const EventChannel* eventInfo, const MidiMessage& event, int samplePosition)
{

	if (triggerEvent < 0) return;
	if (eventInfo->getChannelType() == EventChannel::TEXT && eventInfo == eventChannelArray[triggerEvent])
	{

		TextEventPtr msg = TextEvent::deserializeFromMessage(event, eventInfo);
		//std::cout << "HandleEvent3 - triggerChan_> " << triggerChannel<< std::endl;

		if (msg->getChannel() == triggerChannel)
		{
			String avg = msg->getText();
...
void TTLViewerNode::handleSpike(const SpikeChannel* spikeInfo, const MidiMessage& event, int samplePosition)
{
	...
	bool aboveThreshold = true;// false;
	...
	if (aboveThreshold)
	{
		...
		if (isRecording)
		{
			CoreServices::RecordNode::writeSpike(newSpike, spikeInfo);///////////////// Aqui deberian ir las instrucciones necesarias para hacer lo del avg cuando tenga el booleano del avg... 
		}
		// add to buffer
...
bool TTLViewerNode::checkThreshold (int chan, float thresh, SpikeEvent* s)
{
