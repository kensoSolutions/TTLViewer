...
#ifndef TTLVIEWERCANVAS_H_
#define TTLVIEWERCANVAS_H_
...
class TTLViewerCanvas : public Visualizer, public Button::Listener, public ComboBox::Listener

{
public:
    TTLViewerCanvas(TTLViewerNode* n);
    ~TTLViewerCanvas();
	...
	int getLinesToShowValue();

    void processSpikeEvents();

    void beginAnimation();
    void endAnimation();
	void openSpikePlot(bool select);
...
private:
    ...
    ScopedPointer<TTLViewer> ttlViewer;
    ScopedPointer<Viewport> viewport;
	ScopedPointer<ComboBox> voltageRange;
	//ScopedPointer<ComboBox> timebaseRange;
	ScopedPointer<ComboBox> linesList;
	ScopedPointer<Label> linesLabel;
	StringArray voltageRanges;
	//StringArray timebaseRanges;
	StringArray shownLines;
	ScopedPointer<Label> voltageRangeLabel;
	//ScopedPointer<Label> timebaseRangeLabel;
  ...
  //  SpikeObject spike;
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(TTLViewerCanvas);

};

class TTLViewer : public Component
{
public:
    TTLViewer(TTLViewerCanvas*, Viewport*);
    ~TTLViewer();

    void removePlots();
    void clear();
	void showingSpikesPlots(bool select);
...
private:
    ...
    TTLViewerCanvas* canvas;
    Viewport* viewport;

    OwnedArray<SpikePlot> spikePlots;
	OwnedArray<SpikePlot> spikePlotsSaved;
...
{
public:
    SpikePlot(TTLViewerCanvas*, int elecNum, int plotType, String name_);
    virtual ~SpikePlot();
...
{
public:
    WaveAxes(int channel, int linesToShow);
    ~WaveAxes() {}
